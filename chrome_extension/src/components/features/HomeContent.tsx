...
export function HomeContent({ initialBg }: HomeContentProps) {
    ...
    return (
        <main className="min-h-screen overflow-x-hidden relative">
            ...
            {/* Wallpaper Switch Button - Top Right */}
            <motion.button
            ...
            <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                className="fixed top-8 right-8 z-50 p-3 rounded-full bg-white/10 backdrop-blur-md border border-white/10 text-white/70 hover:text-white hover:bg-white/20 transition-colors shadow-lg"
                title="设置"
            >
            ...
            {/* Fixed Bottom Dock */}
            <CategoryNav categories={categoriesState.categories} />
            ...
            <SettingsPanel
                ...
                onAddCategory={categoriesState.addCategory}
                onUpdateCategory={categoriesState.updateCategory}
                onDeleteCategory={categoriesState.deleteCategory}
                onAddLink={categoriesState.addLink}
                onUpdateLink={categoriesState.updateLink}
                onDeleteLink={categoriesState.deleteLink}
                onDataReloaded={() => window.location.reload()}
            />
        </main>
    );
}
