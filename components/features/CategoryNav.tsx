...
export function CategoryNav() {
    const [activeId, setActiveId] = useState<string>(linkData[0]?.id || "");
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            ...
            if (isBottom) {
                const lastId = linkData[linkData.length - 1]?.id;
    ...
    return (
        <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-[95%] md:w-auto md:max-w-fit pointer-events-none">
            <div className="pointer-events-auto w-full md:w-auto overflow-x-auto no-scrollbar bg-black/60 md:bg-black/40 backdrop-blur-2xl md:backdrop-blur-md border border-white/10 rounded-2xl md:rounded-full shadow-2xl transition-all duration-300 p-2 md:p-2">
                <ul className="flex items-center justify-between md:justify-center gap-2 md:gap-1 px-2 md:px-0 min-w-full md:min-w-0">
                    {linkData.map((category) => (
                        <li key={category.id} className="relative z-10 flex-shrink-0 flex-1 md:flex-none text-center">
                            <button
                                onClick={() => scrollToCategory(category.id)}
                                className={cn(
                                    "relative w-full md:w-auto px-1 py-3 md:px-4 md:py-2.5 rounded-xl md:rounded-full transition-all duration-300 flex flex-col md:flex-row items-center justify-center gap-1.5 md:gap-2",
